...
class ChatCubit extends Cubit<AiModelState> {
  ...
  ChatCubit({required String name})
      : super(AiModelState(
          chatListKey: GlobalKey<AnimatedListState>(),
          name: name,
        )) {
    _setupModel();
  ...
  void _setupModel() async {
    _geminiModel = GenerativeModel(
      apiKey: geminiApiKey,
      model: state.name == 'Heheh' ? 'gemini-1.5-flash-latest' : 'gemini-pro',
    );
    final contents = [
      Content.text(
...
const String chatGPT =
    'sk-proj-59s2eQ5nsnw7FB35SWk8T3BlbkFJKCNqGZ0bTLfwhEEVRHlv';
